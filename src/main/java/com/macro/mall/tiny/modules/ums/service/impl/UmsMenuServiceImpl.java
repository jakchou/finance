...
public class UmsMenuServiceImpl extends ServiceImpl<UmsMenuMapper, UmsMenu> implements UmsMenuService {
    ...
    public List<UmsMenuNode> treeList() {
        List<UmsMenuNode> result = hazelcastInstance.getList(Constant.CACHE_LIST_NAME+Constant.MODEL_MENU);
        if(CollectionUtils.isEmpty(result)){
          List<UmsMenu> menuList =list();
            result = menuList.stream()
                    .filter(menu -> menu.getParentId().equals(0L))
                    .map(menu -> covertMenuNode(menu, menuList)).collect(Collectors.toList());
            hazelcastInstance.getList(Constant.CACHE_LIST_NAME+Constant.MODEL_MENU).addAll(result);
        }
        return result;
    }
